...
img = cv2.imread('D:/Diplomarbeit/Bildsatz/Mono/handle_bbgf_12.5_pantex/view_point_0.bmp')

'''
mask = np.zeros(img.shape[:2],np.uint8)
bgdModel = np.zeros((1,65),np.float64)
fgdModel = np.zeros((1,65),np.float64)
rect = (300,700,1800,2200)#划定区域
# rect = (y1, x1, y2, x2) 从左到右，从上到下
...
if __name__ == "__main__":
    ...
    Images = ['D:/Diplomarbeit/Bildsatz/Mono/handle_bbgf_12.5_pantex/view_point_0.bmp']
            ...
            if noise == noise_gaussian:
                for noise_level in Noise_Levels_Gaussian:
                    ...
                    for filter_wavelet in Filters:
                        if filter_wavelet == filter_haar:
                            success, img_original_haar_edges = pipeline(img_original, filter_wavelet, 'Original Image',threshold_original_haar)
                            success, img_gaussian_haar_edges = pipeline(img_noisy_gaussian, filter_wavelet, noise,threshold_gaussian_haar)

                        elif filter_wavelet == filter_coiflet:
                            success, img_original_coiflet_edges = pipeline(img_original, filter_wavelet,'Original Image',threshold_original_coiflet)
                            success, img_gaussian_coiflet_edges = pipeline(img_noisy_gaussian, filter_wavelet, noise,threshold_gaussian_coiflet)
                ...
                for noise_level in Noise_Levels_Salt_Pepper:
                    ...
                    title = "%s (Salt vs Pepper: %s, Amount: %s)" % (noise, "{0:.2f}".format(noise_level[0]), "{0:.3f}".format(noise_level[1]))
                    ...
                    for filter_wavelet in Filters:
                        if filter_wavelet == filter_haar:
                            success, img_salt_pepper_haar_edges = pipeline(img_noisy_salt_pepper, filter_wavelet, noise,threshold_snp_haar)

                        elif filter_wavelet == filter_coiflet:
                            success, img_salt_pepper_coiflet_edges = pipeline(img_noisy_salt_pepper, filter_wavelet,noise, threshold_snp_coifflet)

                        if success:
                            print(
                                'Wavelet Edge Detection Pipeline for ' + image + ' with ' + noise + ' and ' + filter_wavelet + ' executed successfully')

        printFinalComparisonResults(img_original_haar_edges, img_original_coiflet_edges, img_gaussian_haar_edges,img_gaussian_coiflet_edges, img_salt_pepper_haar_edges,img_salt_pepper_coiflet_edges)
'''
